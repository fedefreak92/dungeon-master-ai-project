...
class TavernaState(BaseState):
    def __init__(self):
        ...
        self.oggetti_interattivi = {
            "bancone": OggettoInterattivo(
                "Bancone",
                "Un lungo bancone di legno lucido dove vengono servite le bevande.",
                "pulito",
                posizione="taverna",
            ),
            "camino": OggettoInterattivo(
                "Camino",
                "Un grande camino in pietra con un fuoco scoppiettante.",
                "acceso",
                posizione="taverna",
            ),
            "baule_nascondiglio": Baule(
                "Baule nascosto",
                "Un piccolo baule nascosto sotto una tavola del pavimento.",
                contenuto=[
                    Oggetto(
                        "Chiave rugginosa",
                        "chiave",
            ...
            "porta_cantina": Porta(
                "Porta della cantina",
                "Una robusta porta che conduce alla cantina.",
                stato="chiusa",
                richiede_chiave=True,
                posizione="taverna",
                posizione_destinazione="cantina",
            ),
            "trappola_pavimento": Trappola(
                "Trappola nel pavimento",
                "Una parte del pavimento sembra instabile.",
                danno=5,
                posizione="taverna",
        ...
        leva_segreta = Leva(
            "Leva segreta", "Una leva nascosta dietro un quadro.", posizione="taverna"
        )
        self.oggetti_interattivi["leva_segreta"] = leva_segreta
        leva_segreta.collega_oggetto(
            "trappola", self.oggetti_interattivi["trappola_pavimento"]
        )

        # Esempio di altare magico
        altare = OggettoInterattivo(
            "Altare magico",
            "Un antico altare con simboli misteriosi.",
            stato="inattivo",
        )
        altare.imposta_descrizione_stato(
        ...
        altare.imposta_descrizione_stato(
            "esaminato",
            "Noti che l'altare ha delle rune che formano un incantesimo antico.",
        )
        ...
        altare.collega_evento(
            "attivo", lambda gioco: gioco.sblocca_area("cripta_magica")
        )
    ...
    def esegui(self, gioco):
        if self.prima_visita:
            gioco.io.mostra_messaggio(
                f"Benvenuto {gioco.giocatore.nome} sei appena arrivato nella Taverna Il Portale Spalancato a Waterdeep. Sei di ritrono da un lungo viaggio che ti ha permesso di ottenere molti tesori ma anche molte cicatrici. Entri con passo svelto e ti dirigi verso la tua prossima avventura"
            )
            ...
            if hasattr(gioco, "gestore_mappe"):
                if mappa := gioco.gestore_mappe.ottieni_mappa("taverna"):
        ...
        if scelta == "1":
            ...
            )  # Usiamo push_stato per il combattimento
        elif scelta == "5":
