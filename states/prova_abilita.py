...
class ProvaAbilitaState(BaseState):
    ...
    def _prova_base(self, gioco, abilita):
        ...
        try:
            difficolta = int(
                gioco.io.richiedi_input("Inserisci la difficoltà (5-20): ")
            )
            ...
            gioco.io.mostra_messaggio(
                f"{gioco.giocatore.nome} tira un {tiro} + {modificatore} ({abilita}) = {risultato}"
            )
    ...
    def _prova_npg(self, gioco, abilita):
        ...
        if not stato_precedente or not hasattr(stato_precedente, "npg_presenti"):
            gioco.io.mostra_messaggio(
                "Non ci sono personaggi non giocanti nelle vicinanze."
            )
            return
        ...
        if isinstance(stato_precedente.npg_presenti, dict):
            ...
            try:
                ...
                if 1 <= scelta <= len(stato_precedente.npg_presenti):
                    ...
                    if hasattr(gioco.giocatore, f"{abilita}_base") and hasattr(
                        npg, f"{abilita}_base"
                    ):
                        g_valore = getattr(gioco.giocatore, f"{abilita}_base")
                        n_valore = getattr(npg, f"{abilita}_base")
                        gioco.io.mostra_messaggio(
                            f"{gioco.giocatore.nome}: {abilita.capitalize()} {g_valore} (mod: {modificatore})"
                        )
                    ...
                    gioco.io.mostra_messaggio(
                        f"Difficoltà contro {npg.nome}: {difficolta}"
                    )

                    if risultato >= difficolta:
                        gioco.io.mostra_messaggio(
                            f"Hai superato la prova di {abilita} contro {npg.nome}!"
                        )
                        self._gestisci_successo_npg(gioco, abilita, npg)
                    else:
                        gioco.io.mostra_messaggio(
                            f"Hai fallito la prova di {abilita} contro {npg.nome}."
                        )
    ...
    def _prova_oggetto(self, gioco, abilita):
        ...
        if not stato_precedente or not hasattr(stato_precedente, "oggetti_interattivi"):
            gioco.io.mostra_messaggio(
                "Non ci sono oggetti interattivi nelle vicinanze."
            )
            return
        ...
        if isinstance(stato_precedente.oggetti_interattivi, dict):
            ...
            try:
                ...
                if 1 <= scelta <= len(stato_precedente.oggetti_interattivi):
                    oggetto_nome = list(stato_precedente.oggetti_interattivi.keys())[
                        scelta - 1
                    ]
                    oggetto = stato_precedente.oggetti_interattivi[oggetto_nome]

                    self.skill_check(gioco, abilita, oggetto)
    ...
    def skill_check(self, gioco, abilita, oggetto):
        """Performs a skill check against an interactive object.

        Calculates the difficulty based on the object and skill, rolls a d20,
        and handles the outcome of the check.

        Args:
            gioco: The game object.
            abilita: The skill being used.
            oggetto: The interactive object being tested against.
        """
        ...
        if isinstance(oggetto, OggettoInterattivo):
            ...
            elif abilita == "destrezza" and hasattr(
                oggetto, "difficolta_salvezza"
            ):
        ...
        gioco.io.mostra_messaggio(
            f"Difficoltà per {oggetto.nome}: {difficolta}"
        )
        ...
        if risultato >= difficolta:
            gioco.io.mostra_messaggio(
                f"Hai superato la prova di {abilita} con {oggetto.nome}!"
            )
            self._gestisci_successo(gioco, abilita)
            ...
            if (
                abilita == "forza"
                and hasattr(oggetto, "stato")
                and oggetto.stato == "integro"
            ):
                gioco.io.mostra_messaggio(
                    f"Grazie alla tua forza, puoi interagire efficacemente con {oggetto.nome}!"
                )
                oggetto.interagisci(gioco.giocatore)
        else:
            gioco.io.mostra_messaggio(
                f"Hai fallito la prova di {abilita} con {oggetto.nome}."
            )
            self._gestisci_fallimento(gioco, abilita)
    ...
    def _gestisci_successo_npg(self, gioco, abilita, npg):
        ...
        if abilita in ABILITA_ASSOCIATE:
            ...
            elif abilita in ["percezione", "indagare"]:
                gioco.io.mostra_messaggio(
                    f"Hai notato qualcosa di importante su {npg.nome}!"
                )
        elif abilita == "forza":
            gioco.io.mostra_messaggio(
                f"Hai impressionato {npg.nome} con la tua forza!"
            )
        elif abilita == "destrezza":
            gioco.io.mostra_messaggio(f"{npg.nome} è colpito dalla tua agilità!")
        elif abilita == "carisma":
            gioco.io.mostra_messaggio(
                f"{npg.nome} è affascinato dalla tua presenza!"
            )
            if hasattr(npg, "stato_corrente"):
        ...
        if gioco.giocatore.guadagna_esperienza(exp):
            gioco.io.mostra_messaggio(
                f"Hai guadagnato {exp} punti esperienza e sei salito di livello!"
            )
        else:
    ...
    def _gestisci_fallimento_npg(self, gioco, abilita, npg):
        ...
        if abilita == "forza":
            gioco.io.mostra_messaggio(
                f"{npg.nome} non sembra impressionato dalla tua forza."
            )
        elif abilita == "destrezza":
            gioco.io.mostra_messaggio(f"{npg.nome} ha notato la tua goffaggine.")
        elif abilita == "carisma":
            gioco.io.mostra_messaggio(
                f"{npg.nome} sembra infastidito dal tuo approccio."
            )
            if hasattr(npg, "stato_corrente"):
    ...
    def _gestisci_successo(self, gioco, abilita):
        ...
        if self.contesto.get("tipo") == "oggetto":
            if oggetto := self.contesto.get("oggetto"):
                gioco.io.mostra_messaggio(
                    f"Sei riuscito a interagire correttamente con {oggetto.nome}!"
                )
    ...
    def _gestisci_fallimento(self, gioco, abilita):
        ...
        if self.contesto.get("tipo") == "oggetto":
            if oggetto := self.contesto.get("oggetto"):
                gioco.io.mostra_messaggio(
                    f"Non sei riuscito a interagire correttamente con {oggetto.nome}."
                )
    ...
    def _prova_abilita_specifica(self, gioco):
        ...
        for i, abilita in enumerate(ABILITA_ASSOCIATE.keys(), 1):
            ...
            competenza = (
                " [Competente]"
                if gioco.giocatore.abilita_competenze.get(abilita.lower())
                else ""
            )
    ...
    def _prova_abilita_base(self, gioco, nome_abilita):
        ...
        try:
            difficolta = int(
                gioco.io.richiedi_input("Inserisci la difficoltà (5-25): ")
            )
            ...
            modificatore_base = getattr(
                gioco.giocatore, f"modificatore_{caratteristica}"
            )
            ...
            if competenza:
                gioco.io.mostra_messaggio(
                    f"Ha competenza in {nome_abilita.capitalize()} (bonus: +{bonus_comp})"
                )
            gioco.io.mostra_messaggio(
                f"Tira un {tiro} + {modificatore_base} (mod) {f'+{str(bonus_comp)}' if bonus_comp else ''} = {risultato}"
            )
    ...
    def _prova_abilita_npg(self, gioco, nome_abilita):
        ...
        if isinstance(stato_precedente.npg_presenti, dict):
            ...
            try:
                scelta = int(gioco.io.richiedi_input("\nScegli NPG: "))
                if 1 <= scelta <= len(stato_precedente.npg_presenti):
                    self.skill_check_npc(
                        stato_precedente, scelta, nome_abilita, gioco
                    )
                else:
                    gioco.io.mostra_messaggio("Scelta non valida.")
            except ValueError:
            ...
            gioco.io.mostra_messaggio(
                "Funzionalità per liste di NPG non ancora implementata completamente."
            )

    def skill_check_npc(self, stato_precedente, scelta, nome_abilita, gioco):
        """
        Handles the skill check process for a non-player character (NPC) in the game.
        Args:
            stato_precedente: The previous state of the game, containing information about the NPCs present.
            scelta: An integer representing the player's choice of NPC to interact with.
            nome_abilita: The name of the skill being used for the check (e.g., "furtività", "persuasione").
            gioco: The game instance, providing access to the player, dice rolls, and input/output methods.
        Behavior:
            - Determines the NPC involved in the skill check based on the player's choice.
            - Calculates the difficulty of the skill check based on the skill type and the NPC's attributes.
            - Rolls a 20-sided die (D20) and adds the player's skill modifier to determine the result.
            - Displays detailed information about the skill check, including the player's base attribute, 
              modifiers, and whether they have proficiency in the skill.
            - Compares the result of the skill check against the calculated difficulty.
            - If the check succeeds, calls a method to handle the success scenario.
            - If the check fails, calls a method to handle the failure scenario.
        """
        npg_nome = list(stato_precedente.npg_presenti.keys())[scelta - 1]
        npg = stato_precedente.npg_presenti[npg_nome]
        ...
        caratteristica_base = getattr(
            gioco.giocatore, f"{caratteristica}_base"
        )
        mod_caratteristica = getattr(
            gioco.giocatore, f"modificatore_{caratteristica}"
        )
        competenza = gioco.giocatore.abilita_competenze.get(
            nome_abilita, False
        )
        bonus_comp = gioco.giocatore.bonus_competenza if competenza else 0
        ...
        gioco.io.mostra_messaggio(
            f"Tira un {tiro} + {modificatore} = {risultato}"
        )
        gioco.io.mostra_messaggio(
            f"Difficoltà contro {npg.nome}: {difficolta}"
        )
        ...
        if risultato >= difficolta:
            gioco.io.mostra_messaggio(
                f"Hai superato la prova di {nome_abilita} contro {npg.nome}!"
            )
            self._gestisci_successo_npg(gioco, nome_abilita, npg)
        else:
            gioco.io.mostra_messaggio(
                f"Hai fallito la prova di {nome_abilita} contro {npg.nome}."
            )
    ...
    def _prova_abilita_oggetto(self, gioco, nome_abilita):
        ...
        if isinstance(stato_precedente.oggetti_interattivi, dict):
            ...
            # Implementazione per liste simile a quella sopra
            gioco.io.mostra_messaggio(
                "Funzionalità per liste di oggetti non ancora implementata completamente."
            )

    def skill_check_object(self, stato_precedente, scelta, nome_abilita, gioco):
        """
        Handles the skill check for interacting with an object in the game.
        Args:
            stato_precedente (Stato): The previous state of the game, containing interactive objects.
            scelta (int): The player's choice, corresponding to the index of the object to interact with.
            nome_abilita (str): The name of the skill being used for the check.
            gioco (Gioco): The game instance, providing access to the player, IO, and other game mechanics.
        Behavior:
            - Determines the object to interact with based on the player's choice.
            - Calculates the difficulty of the skill check, defaulting to 12 if not specified.
            - Rolls a 20-sided die (D20) and adds the player's skill modifier to determine the result.
            - Displays detailed information about the skill check, including the player's attributes and bonuses.
            - Updates the context to indicate the type of interaction and the object involved.
            - If the skill check succeeds, handles the success and triggers the object's specific interaction logic.
            - If the skill check fails, handles the failure and provides feedback to the player.
        Side Effects:
            - Outputs messages to the game's IO system to inform the player of the skill check details and results.
            - Modifies the game's context to reflect the outcome of the interaction.
        """
        oggetto_nome = list(stato_precedente.oggetti_interattivi.keys())[
            scelta - 1
        ]
        ...
        difficolta = (
            oggetto.difficolta_abilita[nome_abilita]
        ...
        if risultato >= difficolta:
            gioco.io.mostra_messaggio(
                f"Hai superato la prova di {nome_abilita} con {oggetto.nome}!"
            )
            self._gestisci_successo(gioco, nome_abilita)

            # Usa il nuovo sistema di interazione specifica
            oggetto.interagisci_specifico(
                gioco.giocatore, nome_abilita, gioco
            )
        else:
            gioco.io.mostra_messaggio(
                f"Hai fallito la prova di {nome_abilita} con {oggetto.nome}."
            )
            self._gestisci_fallimento(gioco, nome_abilita)
