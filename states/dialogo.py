...
class DialogoState(BaseState):
            ...
            try:
                ...
                if 1 <= scelta <= len(opzioni):
                    ...
                    if prossimo_stato is None:
                        ...
                        self.conversazione = self.npg.ottieni_conversazione(
                            prossimo_stato
                        )
                        if self.conversazione is None:
                            gioco.io.mostra_messaggio(
                                f"Stato di dialogo '{prossimo_stato}' non trovato per {self.npg.nome}"
                            )
                            self.in_dialogo = False
                            gioco.pop_stato()
                        else:
    ...
    def _applica_effetto(self, gioco, effetto):
        ...
        if isinstance(effetto, str):
            ...
            if tipo == "consegna_oro":
                ...
                oggetto_trovato = next(
                    (item for item in self.npg.inventario if item.nome == oggetto_nome),
                    None,
                )
                ...
                elif gioco.giocatore.oro < costo:
                    gioco.io.mostra_messaggio(
    ...
    def ha_fatto_scelta(self, stato, scelta=None):
        ...
        return True if scelta is None else self.scelte_fatte[stato] == scelta
