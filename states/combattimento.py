...
class CombattimentoState(BaseState):
    def __init__(self, nemico=None, npg_ostile=None):
        ...
        self.turno = 1
        self.avversario = nemico or npg_ostile
    ...
    def _mostra_stato_combattimento(self, giocatore, gioco):
        ...
        gioco.io.mostra_messaggio("\n" + "=" * 60)
        gioco.io.mostra_messaggio(
            f"COMBATTIMENTO: {giocatore.nome} vs {self.avversario.nome}"
        )
        gioco.io.mostra_messaggio(f"Turno: {self.turno}")
        gioco.io.mostra_messaggio(
            f"HP {giocatore.nome}: {giocatore.hp}/{giocatore.hp_max} | HP {self.avversario.nome}: {self.avversario.hp}/{self.avversario.hp_max}"
        )
        ...
        accessori = (
            ", ".join([a.nome for a in giocatore.accessori])
            if giocatore.accessori
            else "Nessuno"
        )

        gioco.io.mostra_messaggio(
            f"Arma: {arma} | Armatura: {armatura} | Accessori: {accessori}"
        )
        gioco.io.mostra_messaggio(
            f"Forza: {giocatore.forza} | Difesa: {giocatore.difesa}"
        )
    ...
    def _controlla_fine_combattimento(self, gioco):
        ...
        if self.avversario.hp <= 0:
            return self.messaggio_combattimento(gioco, giocatore)
        return False

    def messaggio_combattimento(self, gioco, giocatore):
        ...
        if (
            hasattr(self.avversario, "inventario")
            and self.avversario.inventario
            and len(self.avversario.inventario) > 0
        ):
        ...
        # Guadagna esperienza
        exp_guadagnata = 25 * (
            1 + self.avversario.livello if hasattr(self.avversario, "livello") else 1
        )
        if giocatore.guadagna_esperienza(exp_guadagnata):
            gioco.io.mostra_messaggio(
                f"Hai guadagnato {exp_guadagnata} punti esperienza e sei salito di livello!"
            )
        else:
            gioco.io.mostra_messaggio(
                f"Hai guadagnato {exp_guadagnata} punti esperienza!"
            )
    ...
    def _attacco_nemico(self, giocatore, gioco):
        ...
        if self.npg_ostile:
            danno = max(
                1, self.npg_ostile.forza if hasattr(self.npg_ostile, "forza") else 3
            )
        else:
            danno = (
                self.nemico.forza
                if hasattr(self.nemico, "forza")
                else self.nemico.danno
            )
    ...
    def _applica_effetto_oggetto(self, giocatore, item, gioco):
        ...
        if "Pozione" in item:
            ...
            self.applica(5, gioco, item, giocatore)
        elif "Bomba" in item:
            self.applica(15, gioco, item, giocatore)
        else:
            gioco.io.mostra_messaggio(f"\nNon sai come usare {item} in combattimento!")

    def applica(self, arg0, gioco, item, giocatore):
        # Avvelena il nemico
        danno = arg0
        self.avversario.ferisci(danno)
        gioco.io.mostra_messaggio(
    ...
    def _cambia_arma(self, giocatore, gioco):
        ...
        for i, arma in enumerate(armi, 1):
            bonus = (
                f" (Forza +{arma.effetto.get('forza', 0)})"
                if hasattr(arma, "effetto")
                else ""
            )
    ...
    def _cambia_armatura(self, giocatore, gioco):
        ...
        for i, armatura in enumerate(armature, 1):
            bonus = (
                f" (Difesa +{armatura.effetto.get('difesa', 0)})"
                if hasattr(armatura, "effetto")
                else ""
            )
