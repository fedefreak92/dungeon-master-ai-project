...
class TerminalIO(GameIO):
    ...
    def _output(self, testo: str):
        """
        Metodo interno per gestire l'output verso il terminale.
        Questo metodo può essere sovrascritto per cambiare il comportamento di output.
        
        Args:
            testo (str): Il testo da mostrare
        """
        print(testo)
    
    def _input(self, prompt: str = "") -> str:
        """
        Metodo interno per gestire l'input dal terminale.
        Questo metodo può essere sovrascritto per cambiare il comportamento di input.
        
        Args:
            prompt (str): Il prompt da mostrare
    ...
    def messaggio_sistema(self, testo: str):
        self._output(f"[SISTEMA] {testo}")
        self.buffer.append({"tipo": "sistema", "testo": testo})
        
    def messaggio_errore(self, testo: str):
        self._output(f"[ERRORE] {testo}")
        self.buffer.append({"tipo": "errore", "testo": testo})

    def richiedi_input(self, prompt: str = "") -> str:
        self.ultimo_input = self._input(prompt)
